...
namespace cmft
{
    ...
    size_t rwReadMem(Rw* _rw, void* _data, size_t _size)
    {
		...
		char* read_address = (char*)_rw->m_mem + _rw->m_offset;
		memcpy(_data, (void*)read_address, sizeToRead);
	...
	void imageSaveToMemDDS(const Image& _image, std::vector<char>* mem, ImageFileType::Enum _ft, TextureFormat::Enum _convertTo /*= TextureFormat::Null*/, AllocatorI* _allocator /*= g_allocator*/)
	{
		// Get image in desired format.
		ImageSoftRef image;
		if (TextureFormat::Null != _convertTo)
		...
		DdsHeaderDxt10 ddsHeaderDxt10;
		ddsHeaderFromImage(ddsHeader, &ddsHeaderDxt10, _image);
		size_t dds_size = sizeof(uint32_t);
		dds_size += sizeof(DdsHeader);
		dds_size += ((DDS_DX10 == ddsHeader.m_pixelFormat.m_fourcc) ? sizeof(DdsHeaderDxt10) : 0);
		dds_size += _image.m_dataSize;
		mem->resize(dds_size, 0);
		size_t write_offset = 0;
		// Write magic.
		const uint32_t magic = DDS_MAGIC;
		memcpy(mem->data() + write_offset, &magic, sizeof(uint32_t));
		write_offset += sizeof(uint32_t);

		// Write header.
		memcpy(mem->data() + write_offset, &ddsHeader, sizeof(DdsHeader));
		write_offset += sizeof(DdsHeader);
		
		if (DDS_DX10 == ddsHeader.m_pixelFormat.m_fourcc)
		{
			memcpy(mem->data() + write_offset, &ddsHeaderDxt10, sizeof(DdsHeaderDxt10));
			write_offset += sizeof(DdsHeaderDxt10);
		}

		// Write data.
		DEBUG_CHECK(NULL != _image.m_data, "Image data is null.");
		memcpy(mem->data() + write_offset, _image.m_data, _image.m_dataSize);
